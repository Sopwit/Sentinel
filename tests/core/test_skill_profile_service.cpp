...
void SkillProfileServiceTest::exposesDefaultProfiles() {
    ...
    QVERIFY(profiles.at(0).capabilitySummaries.join(QStringLiteral("\n"))
                .contains(QStringLiteral("Prompt mutation: disabled")));
    QVERIFY(service.profileSummaries().join(QStringLiteral("\n"))
...
void SkillProfileServiceTest::reportsMetadataOnlyReadiness() {
    ...
    QVERIFY(readiness.checks.contains(QStringLiteral("Tool execution and agent activation: disabled")));
    QVERIFY(readiness.developerDiagnostics.contains(QStringLiteral("Runtime authority: unchanged")));
