...
Phase 5.5 records manual visual QA expectations for the current Qt/QML desktop shell. It does not
...
- Rail/Header/Dock: dock navigation selection, hover/focus states, mode selector, compact status
  truncation, and non-interactive local-alpha status cues.
- Mode visuals: Companion, Focus, Mission, System, Minimal, and Tactical mode accent/panel changes
  should remain visual-only and should not imply new runtime capability.
...
- Wide: 1200 px or wider. Dashboard overview and chat should sit side by side without excessive
  crowding.
- Workspace presence: central orbit/readout text should stay inside the presence scene at compact,
  normal, and wide widths, without colliding with the floating dock.
...
- Shared spacing, dock sizing, button height, input height, telemetry readouts, and surfaces look
  consistent.
...
- `qmllint` should be run when available; build-time QML cache compilation remains the fallback
  check when the tool is unavailable.
