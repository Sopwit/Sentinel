...
Phase 52 adds release-candidate packaging and distribution readiness without changing runtime
authority. Build metadata is generated by CMake and exposed through `AppMetadata` to About,
Diagnostics, and user-initiated diagnostics export: app version, numeric project version, build
number, git commit when available, build type, platform, architecture, and Qt version. This
metadata is static local build information only.

Cross-platform package metadata is prepared at the edges of the desktop target: macOS Info.plist
and bundle properties, Windows icon/version resource metadata, Linux desktop/AppStream metadata,
and icon installation. Packaging scripts and docs are operator-invoked helpers only. They do not
sign, notarize, publish, upload, contact update feeds, run package managers silently, or enable
provider/model/tool/agent/voice/runtime behavior.
