...
GitResult runGit(const QString& directory, const QStringList& arguments) {
    ...
    return {success, QString::fromLocal8Bit(output), success ? QString() : QString::fromLocal8Bit(output).trimmed()};
}
}
...
WorktreeService::WorktreeService(QObject* parent) : QObject(parent) {
    m_worktreeBasePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/worktrees";
...
bool WorktreeService::reset(const QString& name) {
    for (auto& wt : m_worktrees) {
        if (wt.name == name) {
            const GitResult result = runGit(wt.path, {QStringLiteral("reset"), QStringLiteral("--hard"), QStringLiteral("HEAD")});
            if (!result.success) setError(result.error.isEmpty() ? result.output : result.error);
...
QList<Worktree> WorktreeService::list() const {
    if (m_repositoryPath.isEmpty()) return m_worktrees;
    const GitResult result = runGit(m_repositoryPath, {QStringLiteral("worktree"), QStringLiteral("list"), QStringLiteral("--porcelain")});
    if (!result.success) {
    ...
    for (const QString& record : records) {
        ...
        for (const QString& line : lines) {
            if (line.startsWith(QStringLiteral("worktree "))) path = line.mid(9).trimmed();
            if (line.startsWith(QStringLiteral("branch refs/heads/"))) branch = line.mid(18).trimmed();
        }
        if (!path.isEmpty())
...
QString WorktreeService::worktreePath() const { return m_worktreeBasePath; }
...
void WorktreeService::setRepositoryPath(const QString& path) {
    ...
    if (repository) m_repositoryPath = repository->path;
    else m_repositoryPath.clear();
}

QString WorktreeService::repositoryPath() const { return m_repositoryPath; }
...
std::optional<GitRepository> WorktreeService::discoverRepository(const QString& path) const {
    ...
    const GitResult result = runGit(absolutePath, {QStringLiteral("rev-parse"), QStringLiteral("--show-toplevel")});
    if (!result.success || result.output.trimmed().isEmpty()) {
        setError(result.error.isEmpty() ? QStringLiteral("Path is not a Git repository") : result.error);
        return std::nullopt;
    }
    const QString root = QDir::cleanPath(result.output.trimmed());
    const GitResult gitDir = runGit(root, {QStringLiteral("rev-parse"), QStringLiteral("--git-dir")});
    return GitRepository{root, gitDir.success ? QDir::cleanPath(gitDir.output.trimmed()) : QString()};
}
...
std::optional<GitStatus> WorktreeService::status(const QString& path) const {
    ...
    const GitResult result = runGit(QFileInfo(path).absoluteFilePath(),
...
QString WorktreeService::currentBranch(const QString& path) const {
    ...
    const GitResult result = runGit(path, {QStringLiteral("branch"), QStringLiteral("--show-current")});
    if (!result.success) {
        setError(result.error);
        return {};
    }
...
std::optional<GitDiffSummary> WorktreeService::diffSummary(const QString& path) const {
    ...
    const GitResult result = runGit(path, {QStringLiteral("diff"), QStringLiteral("--shortstat"), QStringLiteral("HEAD")});
    if (!result.success) {
        setError(result.error);
        return std::nullopt;
    }
    const QString summary = result.output.trimmed();
    const QRegularExpression expression(QStringLiteral("(\\d+) files? changed(?:, (\\d+) insertions?\\(\\+\\))?(?:, (\\d+) deletions?\\(-\\))?"));
    ...
    return GitDiffSummary{match.hasMatch() ? match.captured(1).toInt() : 0,
                           ...
                           match.hasMatch() ? match.captured(3).toInt() : 0,
                           summary};
}

Worktree WorktreeService::createWorktree(const QString& repositoryPath,
    ...
    if (!repository || !validWorktreeName(worktreeName)) {
        if (repository) setError(QStringLiteral("Invalid worktree name"));
        return result;
    }
    ...
    if (branch.isEmpty()) args << QStringLiteral("--detach");
    args << QStringLiteral("--") << target;
    if (branch.isEmpty()) args << QStringLiteral("HEAD");
    else args << branch;
    ...
    result = Worktree{worktreeName, target, branch.isEmpty() ? currentBranch(target) : branch, true};
...
bool WorktreeService::removeWorktree(const QString& repositoryPath, const QString& worktreePath) {
    ...
    const GitResult result = runGit(repository->path, {QStringLiteral("worktree"), QStringLiteral("remove"), QStringLiteral("--force"), QStringLiteral("--"), target});
    if (!result.success) setError(result.error.isEmpty() ? result.output : result.error);
    if (result.success) {
        for (int i = m_worktrees.size() - 1; i >= 0; --i)
            if (m_worktrees[i].path == target) m_worktrees.removeAt(i);
    }
    return result.success;
}

QString WorktreeService::lastError() const { return m_lastError; }
...
void WorktreeService::setError(const QString& error) const { m_lastError = error; }
