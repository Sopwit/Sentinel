...
bool MdnsServiceImpl::publish(const MdnsService& service) {
    ...
    if (service.name.isEmpty() || service.type.isEmpty() || service.port <= 0) return false;
    unpublish(service.name);
#if defined(Q_OS_MACOS)
    auto* process = new QProcess(this);
    process->start(QStringLiteral("dns-sd"), {QStringLiteral("-R"), service.name, service.type,
                                               service.domain.isEmpty() ? QStringLiteral("local") : service.domain,
                                               QString::number(service.port)});
#elif defined(Q_OS_LINUX)
    auto* process = new QProcess(this);
    process->start(QStringLiteral("avahi-publish-service"), {service.name, service.type,
                                                              QString::number(service.port)});
#else
    return false;
#endif
    if (!process->waitForStarted(2000)) { process->deleteLater(); return false; }
...
QList<MdnsService> MdnsServiceImpl::discoveredServices() const {
    ...
    QProcess process;
    process.start(QStringLiteral("avahi-browse"), {QStringLiteral("-rt"), QStringLiteral("_sentinel._tcp")});
    if (process.waitForFinished(3000)) {
        const QString output = QString::fromUtf8(process.readAllStandardOutput());
        for (const QString& line : output.split('\n')) {
            const QStringList fields = line.split(';');
            if (fields.size() < 9 || fields[0] != QStringLiteral("=") || fields[1] != QStringLiteral("*") ) continue;
            bool ok = false;
            const int port = fields[8].toInt(&ok);
            if (ok) m_discovered.append({fields[3], fields[4], fields[6], port});
        }
    }
#endif
    return m_discovered;
...
void MdnsServiceImpl::setEnabled(bool enabled) { m_enabled = enabled; }
bool MdnsServiceImpl::isEnabled() const { return m_enabled; }
