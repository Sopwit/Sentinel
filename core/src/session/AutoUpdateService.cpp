...
UpdateInfo AutoUpdateService::checkForUpdates() const {
    ...
    if (timer.isActive() && reply->error() == QNetworkReply::NoError) {
        ...
        info.releaseDate = QDateTime::fromString(
            object.value(QStringLiteral("releaseDate")).toString(), Qt::ISODate);
...
bool AutoUpdateService::downloadUpdate(const QString& version) {
    ...
    QFile file(path);
    if (!file.open(QIODevice::WriteOnly) || file.write(artifact) != artifact.size())
        return false;
    file.close();
...
bool AutoUpdateService::applyUpdate() {
    ...
    if (m_downloadedArtifact.isEmpty() || updater.isEmpty() ||
        !QFileInfo::exists(m_downloadedArtifact))
        return false;
    return QProcess::startDetached(updater, {m_downloadedArtifact});
}

void AutoUpdateService::setAutoCheck(bool enabled) {
    m_autoCheck = enabled;
}
bool AutoUpdateService::autoCheckEnabled() const {
    return m_autoCheck;
}
