...
std::optional<DurableEvent> DurableEventLog::append(const QString& aggregateId, const QString& type,
    ...
    QSqlQuery query(database_);
    query.prepare(QStringLiteral("SELECT COALESCE(MAX(sequence), 0) + 1 FROM sentinel_events WHERE aggregate_id = ?"));
    ...
    insert.prepare(QStringLiteral("INSERT INTO sentinel_events(aggregate_id, sequence, type, payload) VALUES(?, ?, ?, ?)"));
    ...
    if (!insert.exec()) return std::nullopt;
...
QList<DurableEvent> DurableEventLog::replay(const QString& aggregateId) const {
    ...
    QSqlQuery query(database_);
    query.prepare(QStringLiteral("SELECT sequence, type, payload FROM sentinel_events WHERE aggregate_id = ? ORDER BY sequence"));
    query.addBindValue(aggregateId);
    if (!query.exec()) return events;
    while (query.next()) {
        const QJsonDocument document = QJsonDocument::fromJson(query.value(2).toByteArray());
        events.append({aggregateId, query.value(0).toLongLong(), query.value(1).toString(), document.object()});
    }
    return events;
}
