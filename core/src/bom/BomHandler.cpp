...
QByteArray BomHandler::addBom(const QByteArray& data, BomType type) {
    ...
    switch (type) {
    case BomType::UTF8:
        return QByteArray("\xEF\xBB\xBF", 3) + data;
    case BomType::UTF16LE:
        return QByteArray("\xFF\xFE", 2) + data;
    case BomType::UTF16BE:
        return QByteArray("\xFE\xFF", 2) + data;
    case BomType::UTF32LE:
        return QByteArray("\xFF\xFE\x00\x00", 4) + data;
    case BomType::UTF32BE:
        return QByteArray("\x00\x00\xFE\xFF", 4) + data;
    default:
        return data;
    }
}
