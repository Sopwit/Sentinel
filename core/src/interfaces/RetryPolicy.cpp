...
bool RetryPolicy::shouldRetry(int attempt, int statusCode, const QString& errorString) const {
    ...
    if (isRetryableError(errorString)) return true;
    return false;
}
...
QString RetryPolicy::retryReason(int attempt, int statusCode, const QString& errorString) const {
    if (statusCode == 429) return "Rate limited (429)";
    if (statusCode >= 500 && statusCode < 600) return QStringLiteral("Server error (%1)").arg(statusCode);
    if (errorString.contains("timeout")) return "Request timed out";
...
int RetryPolicy::parseRetryAfter(const QString& header) const {
    ...
    if (ok && seconds > 0) return seconds * 1000;
    ...
    if (date.isValid()) {
        ...
        if (delayMs > 0) return static_cast<int>(delayMs);
    }
    return 0;
}

bool RetryPolicy::isRetryableStatus(int statusCode) const {
    if (m_config.retryOn429 && statusCode == 429) return true;
    if (m_config.retryOn5xx && statusCode >= 500 && statusCode < 600) return true;
    return false;
}

bool RetryPolicy::isRetryableError(const QString& error) const {
    if (!m_config.retryOnNetworkError) return false;
    return error.contains("timeout") || error.contains("network") ||
           error.contains("connection") || error.contains("overload");
}
